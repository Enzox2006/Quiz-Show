const ruota = {
    ...
    _buildAzioni() {
        ...
        let btnGira=this._mkBtn("🎡  GIRA LA RUOTA","#f0c800","#1a0a3c",()=>ruota._giraRuota(),this.attesaLettera||this._expressTurn);
    ...
    _dopoRuota(sp,idx) {
        if (sp.tipo==='bancarotta') {
            ...
            // JOLLY può essere preso una sola volta: sostituisci lo spicchio con 200€
            this.SPICCHI[idx]={label:'200',valore:200,tipo:'euro',colore:'#1e3a8a'};
            ...
            this._playRaddoppia();
            // RADDOPPIA può essere preso una sola volta: sostituisci lo spicchio con BANCAROTTA
            this.SPICCHI[idx]={label:'BANCAROTTA',valore:0,tipo:'bancarotta',colore:'#000000'};
    ...
    _confermaCons(lettera, isRaddoppia) {
        ...
        if (count===0) {
            ...
            setTimeout(()=>{
                ...
                else if (ruota.manche===6 && !ruota.faseGong) { ruota._renderGioco(); main.current="RuotaGioco"; }
    ...
    _vinceRound(idx) {
        ...
        let btn=document.createElement("button");
        btn.innerHTML=this.manche<6?"PROSSIMA MANCHE →":"VERDETTO FINALE →";
    ...
    _renderBonusSelezione() {
        ...
        wrap.appendChild(prizeOval); wrap.appendChild(tabEl); wrap.appendChild(bottomRow);
        field.appendChild(wrap);
    ...
    _renderBonusGioco() {
        ...
        let wrap = document.createElement("div");
        wrap.style.cssText = `position:absolute;top:64px;left:0;right:0;bottom:0;display:flex;flex-direction:column;align-items:center;padding:18px 40px 10px;gap:14px;overflow:hidden;`;

        // Top row: timer (left) + prize oval (center, symmetrical)
        let topRow = document.createElement("div");
        topRow.style.cssText = `display:grid;grid-template-columns:1fr auto 1fr;align-items:center;width:100%;flex-shrink:0;gap:16px;`;
        ...
        timerLabel.style.cssText = `font-family:'Barlow Condensed',sans-serif;font-size:16px;font-weight:700;letter-spacing:3px;color:rgba(255,255,255,0.35);`;
        let timerBox = document.createElement("div");
        timerBox.id = 'bonus-timer';
        timerBox.style.cssText = `background:rgba(0,10,50,0.80);border:1.5px solid rgba(120,180,255,0.45);border-radius:10px;padding:4px 18px;font-family:'Barlow Condensed',sans-serif;font-size:64px;font-weight:800;color:#8dc8ff;min-width:72px;text-align:center;line-height:1.05;flex-shrink:0;`;
        ...
        prizeOval.style.cssText = `background:#1a4fc4;border-radius:50px;padding:12px 48px;font-family:'Barlow Condensed',sans-serif;font-size:44px;font-weight:800;color:white;border:3px solid #4488ff;box-shadow:0 0 40px rgba(68,136,255,0.5);white-space:nowrap;`;
        prizeOval.textContent = this._fmtEuro(this._raddoppioMancheScore);

        let gridRight = document.createElement("div");
        topRow.appendChild(timerWrap); topRow.appendChild(prizeOval); topRow.appendChild(gridRight);

        // Tabellone
        let tabEl = this._buildTabellone();
        let sc = Math.min(0.60, window.innerWidth * 0.70 / 1100);
        tabEl.style.transform = `scale(${sc})`;
        tabEl.style.transformOrigin = 'top center';
        tabEl.style.marginBottom = Math.round((sc - 1) * 260) + 'px';
        tabEl.style.flexShrink = '0';

        // Tema box: category + revealed letters (foto 3 style)
        let temaBox = document.createElement("div");
        temaBox.style.cssText = `background:rgba(4,8,36,0.88);border-top:2px solid rgba(90,140,255,0.28);padding:14px 40px 16px;display:flex;flex-direction:column;gap:10px;align-self:stretch;flex-shrink:0;`;
        let temaTopRow = document.createElement("div");
        temaTopRow.style.cssText = `display:flex;align-items:baseline;gap:16px;`;
        let temaCatLabel = document.createElement("div");
        temaCatLabel.textContent = 'TEMA';
        temaCatLabel.style.cssText = `font-family:'Barlow Condensed',sans-serif;font-size:16px;font-weight:700;letter-spacing:4px;color:rgba(255,255,255,0.3);`;
        let temaLabel = document.createElement("div");
        temaLabel.textContent = this._bonusCat.toUpperCase();
        temaLabel.style.cssText = `font-family:'Barlow Condensed',sans-serif;font-size:24px;font-weight:800;color:#f0c800;letter-spacing:4px;`;
        temaTopRow.appendChild(temaCatLabel); temaTopRow.appendChild(temaLabel);
        let lettersRow = document.createElement("div");
        lettersRow.style.cssText = `display:flex;gap:22px;`;
        for (let l of [...this._raddoppioConsSel, ...(this._raddoppioVocaleSel ? [this._raddoppioVocaleSel] : [])]) {
            let lEl = document.createElement("div");
            lEl.textContent = l;
            lEl.style.cssText = `font-family:'Barlow Condensed',sans-serif;font-size:38px;font-weight:800;color:white;letter-spacing:2px;border-bottom:3px solid rgba(255,255,255,0.55);padding-bottom:4px;min-width:26px;text-align:center;`;
            lettersRow.appendChild(lEl);
        }
        temaBox.appendChild(temaTopRow); temaBox.appendChild(lettersRow);

        // Solution button
        let solBtn = document.createElement("button");
        solBtn.innerHTML = "💡  DAI LA SOLUZIONE";
        solBtn.style.cssText = `background:rgba(34,204,102,0.14);color:#22cc66;border:2px solid rgba(34,204,102,0.5);border-radius:14px;padding:20px 60px;font-family:'Barlow Condensed',sans-serif;font-size:38px;font-weight:800;letter-spacing:2px;cursor:pointer;flex-shrink:0;`;
        solBtn.addEventListener('click', () => {
            clearInterval(ruota._raddoppioTimer);
            ruota._apriSoluzioneBonusRaddoppio();
        });

        wrap.appendChild(topRow); wrap.appendChild(tabEl); wrap.appendChild(temaBox); wrap.appendChild(solBtn);
        field.appendChild(wrap);
    ...
    _bonusCorretta() {
        ...
        btn.style.cssText = `background:#f0c800;color:#1a0a3c;border:none;border-radius:14px;padding:24px 100px;font-family:'Barlow Condensed',sans-serif;font-size:38px;font-weight:800;letter-spacing:4px;cursor:pointer;margin-top:8px;`;
        btn.classList.add('btn-primary');
        btn.addEventListener('click', () => { ruota.manche++; if (ruota.manche<=6) ruota._iniziaManche(); else ruota._verdetto(); });
