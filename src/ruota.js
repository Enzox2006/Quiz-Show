const ruota = {
    ...
    _disegnaRuota(canvas, rotation) {
        ...
        for (let i=0;i<n;i++) {
            ...
            ctx.shadowColor = (textColor==='#ffffff'||textColor==='#ffd700') ? 'rgba(0,0,0,0.9)' : 'rgba(255,255,255,0.4)';
            ctx.shadowBlur=4;
    ...
    _giraRuota() {
        ...
        grafica._statusBar("← TORNA AL MENU","RUOTA DELLA FORTUNA",()=>{});
        let wrap=document.createElement("div");
        wrap.style.cssText=`position:absolute;top:64px;left:0;right:0;bottom:0;display:flex;flex-direction:column;align-items:center;justify-content:flex-start;padding-top:20px;gap:18px;`;

        let wheelWrap=document.createElement("div");
        wheelWrap.style.cssText=`position:relative;overflow:hidden;width:min(92vw,880px);height:min(46vw,440px);flex-shrink:0;`;
        let canvas=document.createElement("canvas");
        canvas.width=640;canvas.height=640;
        canvas.style.cssText=`position:absolute;top:0;left:0;width:min(92vw,880px);height:min(92vw,880px);cursor:grab;touch-action:none;`;
        wheelWrap.appendChild(canvas);

        let hintEl=document.createElement("div");
        hintEl.innerHTML="↔  Trascina la ruota per farla girare";
        hintEl.style.cssText=`font-family:'Barlow Condensed',sans-serif;font-size:26px;color:rgba(255,255,255,0.5);letter-spacing:2px;text-align:center;`;
        ...
        wrap.appendChild(risultatoEl);
        wrap.appendChild(continuaBtn);
        field.appendChild(wrap);
    ...
    _prossimaTriplete() {
        ...
        if (this.sottomanche>=3) {
            for (let i=0;i<3;i++) {
                if (this._trileteBonusRound[i]>=3) this.punteggioGioco[i]+=5000;
            }
            this._trileteBonusRound=[0,0,0];
    ...
    _confermaTriplete(corretto, idx) {
        if (corretto) {
            ...
            if (this.sottomanche===2 && this._trileteBonusRound[idx]>=3) {
                this._showToast(`TRIPLETE COMPLETO! ${this.nomi[idx]} vince 5.000 €!`,"#f0c800");
            } else {
                this._showToast(`Corretto! — ${this.nomi[idx]} (${this.sottomanche+1}/3)`,"#22cc66");
            }
        }
        this.sottomanche++;
    ...
    _giraRuotaFinale() {
        ...
        wheelWrapF.style.cssText=`position:relative;overflow:hidden;width:620px;height:310px;flex-shrink:0;`;
        let canvas=document.createElement("canvas");
        canvas.width=640;canvas.height=640;
        canvas.style.cssText=`position:absolute;top:0;left:0;width:620px;height:620px;`;
